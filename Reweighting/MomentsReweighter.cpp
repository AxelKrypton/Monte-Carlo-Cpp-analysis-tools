...
void MomentsReweighterAbstract::calculateAndSetReweightedMomentsAndMomentsEstimators()
{
    ...
    if (momentsReweighterHelper.errorMethod == jackknife) {
        ...
        std::valarray<std::vector<std::vector<realFloat>>> jackknifeEstimators(
            std::vector<std::vector<realFloat>>(numberOfNewPoints, std::vector<realFloat>(numberOfObservablesToBeReweighted)),
            numberOfBinsUsedToBinData);
        // For the case reweightProbabilityDistribution=false the following  is a small waste of memory
        std::valarray<std::vector<std::vector<Histogram>>> histogramJackknifeEstimators(
        ...
        for (size_t i = 0; i < numberOfBinsUsedToBinData; i++) {
            ...
            jackknifeEstimators[i] = calculateReweightedObservableValues(true, i, &logZAtSimulatedPointsUsingUncorrDataLeavingOutOneEntry,
                                                                         &logZAtNewPointsUsingUncorrDataLeavingOutOneEntry,
                                                                         &histogramJackknifeEstimators[i]);
            smartGuessForLogZ = logZAtSimulatedPointsUsingUncorrDataLeavingOutOneEntry;
        }
        std::cout << "   ...done!\n";
        restoreObservablesAfterReweighting(
            minimumOfEachObservable, &reweightedObservablesFromRawData, &jackknifeEstimators, &histogramJackknifeEstimators);
        ...
        std::valarray<std::vector<std::vector<realFloat>>> bootstrapEstimators(
            std::vector<std::vector<realFloat>>(numberOfNewPoints, std::vector<realFloat>(numberOfObservablesToBeReweighted)),
            *(momentsReweighterHelper.bootstrapNumber));
        // For the case reweightProbabilityDistribution=false the following is a small waste of memory
        std::valarray<std::vector<std::vector<Histogram>>> histogramBootstrapEstimators(
            std::vector<std::vector<Histogram>>(
                numberOfNewPoints,
                std::vector<Histogram>(numberOfObservablesGivenAsInput, Histogram(momentsReweighterHelper.probabilityDistributionBinsize))),
            *(momentsReweighterHelper.bootstrapNumber));
        ...
        for (int iBoot = 0; iBoot < (*(momentsReweighterHelper.bootstrapNumber)); iBoot++) {
            ...
            bootstrapEstimators[iBoot] = calculateReweightedObservableValues(
                true, -1, &logZAtSimulatedPointsUsingUncorrData, &logZAtNewPointsUsingUncorrData, &histogramBootstrapEstimators[iBoot]);
            smartGuessForLogZ = logZAtSimulatedPointsUsingUncorrData;
        }
        std::cout << "   ...done!\n";
        restoreObservablesAfterReweighting(
            minimumOfEachObservable, &reweightedObservablesFromRawData, &bootstrapEstimators, &histogramBootstrapEstimators);
...
void MomentsReweighterAbstract::calculateNewPoints()
{
    ...
    if (useSimulatedPointsAsNewPoints) {
        ...
        for (size_t i = 0; i < newRangesOfParameters.size(); i++) {
            if (newRangesOfParameters[i].first == newRangesOfParameters[i].second)
                ...
                realFloat deltaPar
                    = fabs(newRangesOfParameters[i].first - newRangesOfParameters[i].second) / (newNumberOfPointsOfParameters[i] - 1);
                for (unsigned int j = 0; j < newNumberOfPointsOfParameters[i]; j++)
                    newPointValuesForSingleParameter[i].push_back(std::min(newRangesOfParameters[i].first, newRangesOfParameters[i].second)
                                                                  + j * deltaPar);
            }
        }
    ...
    momentsAtNewPoints = std::vector<std::vector<Moments>>(
        valuesOfNewParameters.size(), std::vector<Moments>(momentsReweighterHelper.numberOfObservablesGivenAsInput, Moments()));
    momentsEstimatorsAtNewPoints = std::vector<std::vector<MomentsEstimators>>(
        valuesOfNewParameters.size(),
        std::vector<MomentsEstimators>(momentsReweighterHelper.numberOfObservablesGivenAsInput, MomentsEstimators()));
    if (momentsReweighterHelper.reweightProbabilityDistribution) {
        probabilityDistributionsAtNewBetas = std::vector<std::vector<Histogram>>(
            valuesOfNewParameters.size(), std::vector<Histogram>(momentsReweighterHelper.numberOfObservablesGivenAsInput,
                                                                 Histogram(momentsReweighterHelper.probabilityDistributionBinsize)));
        probabilityDistributionEstimatorsAtNewBetas = std::vector<std::vector<HistogramEstimator>>(
            valuesOfNewParameters.size(),
            std::vector<HistogramEstimator>(momentsReweighterHelper.numberOfObservablesGivenAsInput,
                                            HistogramEstimator(momentsReweighterHelper.probabilityDistributionBinsize)));
    }
}
...
MomentsReweighterAbstract::calculateLogZAtNewPoints(std::vector<std::vector<realFloat>> valuesOfParametersAtWhichLogZIsCalculated,
        ...
        for (size_t indexSimulation1 = 0; indexSimulation1 < numberOfSimulationsDone; indexSimulation1++) {
            ...
            for (size_t indexConfiguration = 0; indexConfiguration < numberConfigurations1; indexConfiguration++) {
                ...
                for (size_t indexSimulation2 = 0; indexSimulation2 < numberOfSimulationsDone; indexSimulation2++) {
                    ...
                    if (entryToBeLeftOut >= 0)
                        numberConfigurations2--;  // The number of conf has to be decreased if we leave one out!
                    realFloat exponent = 0.0;
                    for (size_t indexConjugatedQuantity = 0; indexConjugatedQuantity < numberOfReweightingParameters;
                         indexConjugatedQuantity++) {
...
std::vector<realFloat> MomentsReweighterAbstract::calculateLogZAtSimulatedPoints(bool useUncorrData, const int entryToBeLeftOut,
                                                                                 std::vector<realFloat>* logZAtSimulationPointToStartFrom,
                                                                                 bool printUserInfo)
{
    /*
    ...
    if (entryToBeLeftOut
        >= momentsReweighterHelper.numberOfBinsToBeUsed[0])  // Here negative values mean do not leave out entry, so no check is done!
        throw std::out_of_range(
            "Invalid entry to be left out in \"calculateLogZAtSimulatedPointsUsingUncorrDataAndLeavingOutOneEntry\" function.");
    ...
    if (printUserInfo) {
        std::cout << "==========================================================\n";
        std::cout << " Calculating LogZ At Simulated Points (precision = " << precisionOfIterativeProcedureToCalculateLogZ << ")..."
                  << std::endl;
    ...
    if (logZAtSimulationPointToStartFrom != &logZAtSimulatedPoints || resultLogZ == std::vector<realFloat>(logZAtSimulatedPoints.size(), 0)) {
        ...
        while (true) {
            ...
            if (sqrt(residuumNew) > precisionOfIterativeProcedureToCalculateLogZ) {
                ...
                if (abs(residuumOld - residuumNew) < pow(10, -NUM_DECIMAL_DIGITS))
                    throw std::runtime_error("Iterative procedure to calculate logZ got stuck after " + std::to_string(iter)
                                             + " iterations with " + boost::lexical_cast<std::string>(residuumNew));
            } else
                break;
        }
        if (printUserInfo) {
        ...
        for (size_t i = 0; i < indicesOfParametersAtWhichLogZHasToBeCalculated.size(); i++)
            valuesOfParametersAtWhichLogZHasToBeCalculated.push_back(
                valuesOfSimulationParameters[indicesOfParametersAtWhichLogZHasToBeCalculated[i]]);
        // Real calculation
        realFloat residuum;
        ...
        while (true) {
            ...
            for (size_t indexSimulations = 0; indexSimulations < valuesOfParametersAtWhichLogZHasToBeCalculated.size(); indexSimulations++) {
                residuum += boost::math::expm1(newLogZ[indexSimulations]
                                               - logZAtSimulatedPoints[indicesOfParametersAtWhichLogZHasToBeCalculated[indexSimulations]])
                            * boost::math::expm1(newLogZ[indexSimulations]
                                                 - logZAtSimulatedPoints[indicesOfParametersAtWhichLogZHasToBeCalculated[indexSimulations]]);
                // here one should put eq.(8.34)
            }
 ...
 */
std::vector<std::vector<realFloat>>
MomentsReweighterAbstract::calculateReweightedObservableValues(bool useUncorrData, const int entryToBeLeftOut,
                                                               std::vector<realFloat>* logZAtSimulationPointToBeUsed,
                                                               ...
                                                               std::vector<std::vector<Histogram>>* histoToBeFilled)
{
    ...
    size_t numberOfSimulationsDone = valuesOfSimulationParameters.size();
    size_t numberOfNewPoints = valuesOfNewParameters.size();
    std::vector<std::vector<realFloat>> outputValuesOfObservables(
            ...
            for (size_t indexConfiguration = 0; indexConfiguration < numberConfigurations1; indexConfiguration++) {
                ...
                if (momentsReweighterHelper.reweightProbabilityDistribution) {
                    for (int indexInputObservable = 0; indexInputObservable < momentsReweighterHelper.numberOfObservablesGivenAsInput;
                        ...
                        realFloat tempRestoredObs
                            = exp(simDataCont[indexSimulation1][columnsInputObservables[indexInputObservable]][indexConfiguration]);
        ...
        for (int indexObservable = numberOfReweightingParameters;
            ...
            if (minimumOfEachObservable[indexObservable - numberOfReweightingParameters] < 0) {
                if (indexSimulation == 0)
                    std::cout << "  Observable number " << indexObservable - numberOfReweightingParameters
                              << " shifted due to negative values!\n";
                momentsReweighterHelper.simulationRawDataContainer[indexSimulation][indexObservable]
                    -= 2 * minimumOfEachObservable[indexObservable - numberOfReweightingParameters];
                momentsReweighterHelper.simulationUncorrDataContainer[indexSimulation][indexObservable]
                    -= 2 * minimumOfEachObservable[indexObservable - numberOfReweightingParameters];
            }
            for (int indexData = 0;
                 indexData < momentsReweighterHelper.simulationRawDataContainer[indexSimulation][indexObservable].getNumberOfElements();
                 indexData++) {
                ...
                momentsReweighterHelper.simulationRawDataContainer[indexSimulation][indexObservable][indexData]
                    = log(momentsReweighterHelper.simulationRawDataContainer[indexSimulation][indexObservable][indexData]);
            }
            for (int indexData = 0;
...
void MomentsReweighterAbstract::restoreObservablesAfterReweighting(
    std::vector<realFloat> minimumOfEachObservable, std::vector<std::vector<realFloat>>* reweightedObservablesFromRawData,
        ...
        for (int indexSimulation = 0; indexSimulation < momentsReweighterHelper.simulationRawDataContainer.getNumberOfDatafiles();
            ...
            for (int indexData = 0;
                ...
                momentsReweighterHelper.simulationRawDataContainer[indexSimulation][numberOfReweightingParameters + indexObservable][indexData]
                    = exp(momentsReweighterHelper
                              .simulationRawDataContainer[indexSimulation][numberOfReweightingParameters + indexObservable][indexData]);
            for (int indexData = 0;
                 indexData
                ...
                momentsReweighterHelper.simulationUncorrDataContainer[indexSimulation][numberOfReweightingParameters + indexObservable][indexData]
                    = exp(momentsReweighterHelper
                              .simulationUncorrDataContainer[indexSimulation][numberOfReweightingParameters + indexObservable][indexData]);
        }
        for (size_t indexNewPoint = 0; indexNewPoint < valuesOfNewParameters.size(); indexNewPoint++) {
            if (estimatorsForErrorCalculation != NULL) {
                for (size_t indexBin = 0; indexBin < (*estimatorsForErrorCalculation).size(); indexBin++) {
                    (*estimatorsForErrorCalculation)[indexBin][indexNewPoint][indexObservable]
                        = exp((*estimatorsForErrorCalculation)[indexBin][indexNewPoint][indexObservable]);
                    if (minimumOfEachObservable[indexObservable] < 0)
                        (*estimatorsForErrorCalculation)[indexBin][indexNewPoint][indexObservable]
                            += 2 * minimumOfEachObservable[indexObservable];
                }
            }
            if (reweightedObservablesFromRawData != NULL) {
                (*reweightedObservablesFromRawData)[indexNewPoint][indexObservable]
                    = exp((*reweightedObservablesFromRawData)[indexNewPoint][indexObservable]);
                if (minimumOfEachObservable[indexObservable] < 0)
        ...
        for (size_t indexNewPoint = 0; indexNewPoint < valuesOfNewParameters.size(); indexNewPoint++) {
            for (int indexInputObservable = 0; indexInputObservable < momentsReweighterHelper.numberOfObservablesGivenAsInput;
                ...
                if (minimumOfEachObservable[columnsInputObservables[indexInputObservable] - reweightingParameterNames.size()] < 0)
                    probabilityDistributionsAtNewBetas[indexNewPoint][indexInputObservable].shift(
                        2 * minimumOfEachObservable[columnsInputObservables[indexInputObservable] - reweightingParameterNames.size()]);
            ...
            for (size_t indexBin = 0; indexBin < (*reweightedHistogramEstimators).size(); indexBin++) {
                for (size_t indexNewPoint = 0; indexNewPoint < valuesOfNewParameters.size(); indexNewPoint++) {
                    for (int indexInputObservable = 0; indexInputObservable < momentsReweighterHelper.numberOfObservablesGivenAsInput;
                        ...
                        if (minimumOfEachObservable[columnsInputObservables[indexInputObservable] - reweightingParameterNames.size()] < 0)
                            (*reweightedHistogramEstimators)[indexBin][indexNewPoint][indexInputObservable].shift(
                                2 * minimumOfEachObservable[columnsInputObservables[indexInputObservable] - reweightingParameterNames.size()]);
...
void MomentsReweighterAbstract::extractAndSetReweightedMomentsAndMomentsEstimators(
    const std::vector<std::vector<realFloat>>& reweightedObservablesFromRawData,
...
void MomentsReweighterAbstract::extractAndSetReweightedHistogramEstimators(
        ...
        for (size_t indexInputObservable = 0; indexInputObservable < numberOfObservablesGivenAsInput; indexInputObservable++) {
            ...
            for (double middleOfBin : middleOfBins) {
                ...
                for (size_t indexEstimator = 0; indexEstimator < numberOfHeightsPerBins; indexEstimator++) {
                    double height
                        = histogramEstimatorsForErrorCalculation[indexEstimator][indexNewPoint][indexInputObservable].getHeightOfSpecificBin(
                            middleOfBin);
 ...
 */
static void
findIflogZHasToBeCalculated(std::vector<realFloat> logZAtSimulatedPoints, std::vector<int>& indicesOfParametersAtWhichLogZHasToBeCalculated)
{
...
static void writeNewPoints(std::vector<std::vector<realFloat>>& valuesOfNewParameters,
                           std::vector<std::vector<realFloat>> newPointValuesForSingleParameter, std::vector<realFloat> aux,
                           int numberOfRow, int atInTheRow)
{
...
{
    return (logx1 >= logx2) ? logx1 + boost::math::log1p(exp(logx2 - logx1)) : logx2 + boost::math::log1p(exp(logx1 - logx2));
}
