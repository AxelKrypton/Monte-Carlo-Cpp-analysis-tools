...
SimulationDataContainer SimulationDataContainer::getUncorrelatedSimulationDataSet(std::vector<int> numberOfBinsToBeUsed,
    ...
    if (errorMethod == jackknife) {
        for (int i = 0; i < uncorrelatedSimulationDataSet.getNumberOfDatafiles(); i++) {
            ...
            for (int j = 0; j < uncorrelatedSimulationDataSet[i].getNumberOfDataSample(); j++) {
                ...
                uncorrelatedSimulationDataSet[i][j]
                    = simulationDataSet[i][j].sampleSlice(0, numberOfBinsToBeUsed[i], tmpSizeOfDataSample / numberOfBinsToBeUsed[i]);
            }
        }
    } else if (errorMethod == bootstrap) {
        if (generator == NULL)
            throw std::invalid_argument(
                "Function \"getUncorrelatedSimulationDataSet\" called without random generator needed for the bootstrap case! Aborting...");
        for (int i = 0; i < uncorrelatedSimulationDataSet.getNumberOfDatafiles(); i++) {
            for (int j = 0; j < uncorrelatedSimulationDataSet[i].getNumberOfDataSample(); j++) {
                // Bin all columns as the first was binned!
                if (j == 0)
                    uncorrelatedSimulationDataSet[i][j]
                        = getRandomlyOneDataPerBinFromDataSample(simulationDataSet[i][j], numberOfBinsToBeUsed[i], false, generator);
                else
                    uncorrelatedSimulationDataSet[i][j]
                        = getRandomlyOneDataPerBinFromDataSample(simulationDataSet[i][j], numberOfBinsToBeUsed[i], true);
            }
        }
    } else {
    ...
    while (std::getline(infile, line)) {
        ...
        if (line.find_first_not_of("#\t ") < line.find_first_of("#")) {
            ...
            if (isLastEntryPresentMoreThanOnce(dataFilenames)) {
                throw std::invalid_argument("Configuration file \"" + fileIn + "\" contains the datafile \"" + dataFilenames.back()
                                            + "\" more than once!");
            }
                                                              ...
                                                              std::default_random_engine* generator)
{
    ...
    if (sameIndecesAsLastTime) {
        if (dataSetIn.getNumberOfElements() != dimensionDataSetLastCall || numberOfBins != numberOfBinsLastCall
            || indecesUsedLastCall.size() == 0)
            throw std::invalid_argument(
                "Invalid call to \"getRandomlyOneDataPerBinFromDataSample\" with sameIndecesAsLastTime==true! Aborting...");
        if (indecesUsedLastCall.size() != (size_t)numberOfBins)
            throw std::runtime_error("In function \"getRandomlyOneDataPerBinFromDataSample\" indecesUsedLastCall is not empty but has "
                                     "wrong size!! Investigate...");
