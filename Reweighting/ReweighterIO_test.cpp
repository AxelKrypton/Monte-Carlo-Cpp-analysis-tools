...
class ReweighterIOTester {
  public:
    ...
    {
        return reweighterIO.valuesOfSimulationParametersIgnoringMetaParameters;
    };
...
BOOST_AUTO_TEST_SUITE(build)
    ...
    BOOST_AUTO_TEST_CASE(build1)
    {
        ...
        std::string fileThatDoesExistButWrong2
            = "GeneralTestFiles/wrong_configfile_4";  // correct structure but with two identical set of parameters
    ...
    BOOST_AUTO_TEST_CASE(build2)
    {
        ...
        std::string fileThatDoesExistButWrong1
            = "GeneralTestFiles/wrong_configfile_7";  // correct structure but with datafile with wrong number of columns
        std::string fileThatDoesExistButWrong2
            = "GeneralTestFiles/wrong_configfile_8";  // correct structure but with different parameters name in two lines
        std::string fileThatDoesExistButWrong3
            = "GeneralTestFiles/wrong_configfile_9";  // correct structure but with different number of observables in one file
        std::string fileThatDoesExistButWrong4
            = "GeneralTestFiles/wrong_configfile_10";  // correct structure but with two identical set of reweigthing parameters
        std::string fileThatDoesExistButWrong5
            = "GeneralTestFiles/wrong_configfile_11";  // correct structure but without binsize at least on one line
        std::string fileThatDoesExistButWrong6 = "GeneralTestFiles/wrong_configfile_12";  // correct structure but with negative binsize
        std::string fileThatDoesExistButWrong7
            = "GeneralTestFiles/wrong_configfile_13";  // correct structure but with different number of observables in two files
        ...
        std::string fileThatDoesExistButWrong9
            = "GeneralTestFiles/wrong_configfile_15";  // correct structure but with negative moment in binsize
    ...
    BOOST_AUTO_TEST_CASE(build4)
    {
        ...
        for (size_t i = 0; i < referenceParValue.size(); i++) {
            for (size_t j = 0; j < referenceParValue[i].size(); j++) {
                BOOST_REQUIRE_EQUAL(
                    boost::lexical_cast<std::string>(reweighterIOTester.getValuesOfSimulationParametersIgnoringMetaParameters()[i][j]),
                    boost::lexical_cast<std::string>(referenceParValue[i][j]));
            }
        }
    }
    ...
    BOOST_AUTO_TEST_CASE(build8)
    {
        ...
        BOOST_REQUIRE(
            (boost::math::isnan)(gottenLogZ[0]));  // parenthesis around boost::math::isnan crucial otherwise the std lib macro is called!
...
//        ReweighterTester(options).getRawDataForReweightingAndMetainformation(momentsToBeReweighted, binsizesToBeUsed);
...
//													   "--newBetaRange_high=5.3509", "--numberOfNewBetaPoints=30"};
//        reweighter = new MomentsReweighter(ReweighterTester(options2).getRawDataForReweightingAndMetainformation(momentsToBeReweighted,
//        binsizesToBeUsed)); for(size_t i=0; i<simulatedLogZ.size(); i++)
