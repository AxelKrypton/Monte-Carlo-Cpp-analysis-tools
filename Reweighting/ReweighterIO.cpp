...
static void
extractNamesOfParametersFromSimulationDataIgnoringMetaParameters(SimulationData, std::vector<std::string>&, const std::vector<std::string>&);
static void extractValuesOfSimulationParametersIgnoringMetaParameters(SimulationDataContainer, std::vector<std::vector<realFloat>>&,
...
ReweighterIO::ReweighterIO(LqcdReweightingParameters parameters)
    : readFromFileDataContainer(parameters.getInputfile())
    , bootstrapNumber(nullptr)
{
...
static void extractNamesOfParametersFromSimulationDataIgnoringMetaParameters(SimulationData simData, std::vector<std::string>& parNames,
 ...
 */
static void extractValuesOfSpecifiedBinsizes(SimulationDataContainer simDataCont, std::vector<Binsizes>& valuesOfBinsizes,
        ...
        for (std::map<std::string, realFloat>::iterator it = auxMap.begin(); it != auxMap.end(); it++) {
            if ((it->first) == binsizeLabel) {
                ...
                if (it->second <= 0)
                    ...
                    valuesOfBinsizes[i][std::stoi(lastPartOfString)]
                        = (int)it->second;  // std::stoi throws an exception if no conversion can be done!
                }
            }
        }
        if (! found)
...
static void
checkCorrectnessOfConfigurationFileForReweighting(SimulationDataContainer simDataCont, const std::vector<std::string> metaPar, int numObs)
{
