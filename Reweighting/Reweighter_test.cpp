...
BOOST_AUTO_TEST_SUITE(privateFunctions)
    ...
    BOOST_AUTO_TEST_CASE(getRewProcedures1)
    {
        std::initializer_list<std::string> options = {"-f./RealTestData/configfile_7", "--useBootstrapAsErrorMethod", "--deactivateReweightingForMean",
                                                      "--deactivateReweightingForVariance", "--deactivateReweightingForSkewness", "--deactivateReweightingForKurtosis"};
...
BOOST_AUTO_TEST_SUITE(bindReweighting)
     ...
     *
     *   x1 = 0.512078816369961    x2 = 0.262237401564598    x3 = 0.134299237823898    x4 = 0.0687818157251345     ->   Kurtosis = 3.74781141215248
     *   x1 = 0.512078816369975    x2 = 0.26223740156461    x3 = 0.134299237823896    x4 = 0.0687818157251355     ->   Kurtosis = 3.74790400766947
     *
    ...
    BOOST_AUTO_TEST_CASE(bindReweighting4)
    {
        ...
        std::vector<std::vector<Observables> > valuesObsNewPoints2 = reweighter.getReweightedObservables();
        for(size_t i=0; i<valuesObsNewPoints1.size(); i++){
           BOOST_REQUIRE_CLOSE(valuesObsNewPoints1[i][0].kurtosis.estimate,
                               valuesObsNewPoints2[2*i][0].kurtosis.estimate, realFloatPrecisionInPercent);
           BOOST_REQUIRE_CLOSE(valuesObsNewPoints1[i][0].kurtosis.error,
                               valuesObsNewPoints2[2*i][0].kurtosis.error, realFloatPrecisionInPercent);
        }
    }
